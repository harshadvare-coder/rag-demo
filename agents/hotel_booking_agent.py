...
class HotelBookingAgent:
    ...
    # ----------------------------------------------------------
    # LLM — streaming delta generator
    # ----------------------------------------------------------

    def _stream_llm(self, system_prompt: str, history: list, user_message: str):
        """Yield raw text delta chunks from the LLM as they arrive."""
        messages = []
        for msg in history:
            messages.append({
                "role":    msg["role"],
        ...
        try:
            ...
            print(f"[BookingAgent] LLM stream failed: {exc}")
            yield (
